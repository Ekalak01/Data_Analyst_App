...
class Datasheet(QtWidgets.QMainWindow):
    ...
    def lineplot(self):
        ...
        if self.dcol == True:
            ...
            line=alt.Chart(self.dfff).transform_fold(
                    ...
                    color=alt.Column('key:N',legend=alt.Legend(title='Line Color')),
    ...
    def barplot(self):
        ...
        if self.dcol == True:
            if len(self.col) >=2:
                for i in range(len(self.row)):
                    ch=alt.Chart(self.dfff).mark_bar().encode(
                    x=str(self.col[1])+':N',
                    y=self.row[i]+':Q',
                    tooltip=[str(self.col[0]),str(self.col[1]),self.row[i]]
                    ).facet(
                        column=alt.Column(str(self.col[0]))
                ...
                for i in range(len(self.row)):
                    for j in self.col:
                        ...
                        x=str(self.col[0])+':N',
                        y=self.row[i]+':Q',
                        tooltip=[str(self.col[0]),self.row[i]]
                        ).interactive()
                        nchart.append(ch)
            ...
            if len(self.row) >=2:
                for i in range(len(self.col)):
                    ...
                    y=str(self.row[1])+':N',
                    x=self.col[i]+':Q',
                    tooltip=[str(self.row[0]),str(self.row[1]),self.col[i]]
                    ).facet(
                        row=alt.Row(str(self.row[0]))
                ...
                for i in range(len(self.col)):
                    ch=alt.Chart(self.dfff).mark_bar().encode(
                        y=str(self.row[0])+':N',
                        x=self.col[i]+':Q',
                        tooltip=[str(self.row[0]),self.col[i]]
                        ).interactive()
                    nchart.append(ch)
        ...
        try:
            ...
            for i in dimension:
                if i in col:
                    self.dcol=True
            if self.dcol==True:
                self.dff=self.dff.groupby(dtcol).sum()
                self.dfff=self.dff.reset_index()
            
            else:
                
                self.dff=self.dff.groupby(dtrow).sum()
                self.dfff=self.dff.reset_index()
    ...
    def WindowsFilterPopup(self, item):
        try:
            self.dfff=self.dff.reset_index()
            exPopup = FilterPopup(item,self.dfff, self)
